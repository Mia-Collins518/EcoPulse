...
1. **Training the Model**
   - Model Architecture: Implements a CNN based on the LeNet architecture for MNIST digit classification. 
   - Model Training: The training loop is a method that accepts parameters that support continued training. 
   - Dataset: Uses PyTorch's ```torchvision``` to download and manage the MNIST dataset for training and evaluation.
...
2. **Checkpoint and Saving System**
   ...
   - Dynamic Directory Management: checkpoints are automatically organized into specific project directories (e.g., ```checkpoints/project_name/```) to prevent overwriting and allow easy comparison of different runs.
      - Flexible naming convention based on script name, model version, and training configuration.
   - Flexible Resuming and Inference: ```load_checkpoint_for_ training```function loads all necessary metadata to resume training runs, and ```load_checkpoint_for_ inference``` function defaults to loading the "best_model" but can load any specified checkpoint for the training process.
...
3. **Metrics Workflow**
   ...
   - Data Conversion: The ```convert.py``` script utilizes ```tbparse``` to automatically transform the ```.tfevent``` files into CSV files, allowing the frontend to utilize the data for display.
...
4. **Full-Stack API Integration**
   ...
   - Metric Discovery: API routes (```/projects```, ```/tags```) dynamically read the metrics directory, providing the frontend with a list of available training runs and metrics without hardcording.
   - Random Inference: The ```/random-inference``` endpoint loads the trained model, randomly selects a test image, runs the prediction, Base64-encodes the input images as a PNG, and returns the actual, predicted, and image data in a single JSON payload.
   - Custom Inference: The ```/custom-inference``` adds a POST API endpoint to handle upload Base64 image from the frontend, then returns the input image sas a PNG, the prediction in a single JSON payload.
        - Future Improvements: The image processing on the image processing doesn't work very well. 
...
1. **Start the Backend API**
   ...
   **Instructions**
   ...
   ```
   - Successful Outcome: you will see the message ```bash Uvicorn running on http://127.0.0.1:8000```
...
## Website Shut Down
   ...
   1. Stop the Frontend Server: Go to the terminal window running ```npm run dev``` and press ```CTRL + C```.
   2. Stop the Backend Server: Go to the terminal window running ```uvicorn backend.main:app``` and press ```CTRL + C```.
